...
# include "ext/mysql/php_mysql_query_summary.h"
...
zend_function_entry mysql_functions[] = {
	...
	PHP_FE(mysql_query_history,     					NULL)  /* MY FUNCTION */
...
static void php_mysql_do_query_general(zval **query, zval **mysql_link, int link_id, zval **db, int use_store, zval *return_value TSRMLS_DC)
{
    ...
    /* mainting my query history */
    MySG(total_queries)++;
    struct php_mysql_query_summary *result = malloc(sizeof(php_mysql_query_summary);
...
/* {{{ proto array mysql_query_history(resource result)
   Gets a list of mysql histories from first to list */
PHP_FUNCTION(mysql_query_history) 
{
  struct php_mysql_query_summary result;
  RETURN_LONG(MySG(total_queries)); 
}
/* }}} */
